...
class CartController extends Controller
{
    ...
    public function getAdd(Request $request) {
        ...
        return redirect('/shop')->with('success', 'Success add to cart');
    }
    ...
    public function getDelete($id) {
        ...
        $cart->delete();
        return redirect('cart')->with('success', '1 barang sudah dihapus');
    }

    public function postCheckout(Request $request) {
        $carts = Carts::getAllBySession();
        if(count($carts) == 0) {
            return redirect()->back()->with('danger', 'Wah keranjang belanjamu masih kosong');
        }
        $total_price = 0;
        ...
        return redirect('/shop')->with('success', 'Success Checkout');
    }
}
